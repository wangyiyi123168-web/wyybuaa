...
from models.mlp import DiagRefineMLP
...
class TFormer(nn.Module):
    def __init__(self,num_classes=5):
        ...
        self.diag_refine = DiagRefineMLP(
            main_hidden=32,
            aux_hidden=16,
            aux_out_dim=8,
            fusion_hidden=32,
    ...
    def forward(self,meta_x,cli_x,der_x):
        ...
        # 用 7 个辅助头 + diag 对 diag 做精修
        refined_diag, refine_aux = self.diag_refine(
            diag, pn, bwv, vs, pig, str, dag, rs
        )

        # 直接把 refined_diag 作为最终主任务输出
        return [refined_diag, pn, bwv, vs, pig, str, dag, rs]
