    ...
    try:
        for e in range(epochs):
            ...
            for batchIndex, (der_data,cli_data,meta_data,target) in enumerate(train_iterator):
                ...
                loss = torch.true_divide(
                    ...
                    + 0.1*model.criterion(output[1],pn_label)
                    + 0.1*model.criterion(output[2],bmv_label)
                    + 0.1*model.criterion(output[3],vs_label)
                    + 0.1*model.criterion(output[4],pig_label)
                    + 0.1*model.criterion(output[5],str_label)
                    + 0.1*model.criterion(output[6],dag_label)
                    + 0.1*model.criterion(output[7],rs_label),8
                )
            ...
            for der_data, cli_data, meta_data, target in valid_iterator:
                ...
                valid_loss = model.criterion(output[0], diagnosis_label)
            ...
            print("Valid loss is:{:.4f},diag accuracy:{:.4f}%".format(avg_valid_loss, accuracy_valid))
            log_file.write("Valid loss is:{:.4f},diag accuracy:{:.4f}%".format(avg_valid_loss, accuracy_valid) + '\n')
        ...
        if complete:
            ...
            for der_data,cli_data,meta_data,target in test_iterator:
                ...
                test_loss = model.criterion(output[0], diagnosis_label)
